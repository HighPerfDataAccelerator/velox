...
#define DEFINE_SIMPLE_GROUPBY_AGGREGATOR(Name, name, KIND)                    \
  struct Groupby##Name##Aggregator : GroupbyAggregator {                      \
    ...
    void addGroupbyRequest(                                                   \
      ...
      if (constant != nullptr) {                                              \
        ...
        if (maskIndex.has_value()) {                                          \
          constant_input = cudf_velox::applyMask(                             \
              constant_input->view(), tbl.column(*maskIndex), stream, mr);    \
        }                                                                     \
    ...
    size_t releaseRequestState() override {                                   \
      const size_t released = GroupbyAggregator::releaseRequestState() +      \
          (constant_input == nullptr ? 0 : 1);                                \
    ...
    bool supportsPartialIdentity() const override {                           \
      return step == core::AggregationNode::Step::kPartial &&                 \
          constant == nullptr && !maskIndex.has_value();                      \
    }                                                                         \
                                                                              \
...
struct GroupbyDecimalSumAggregator : GroupbyAggregator {
  ...
  size_t releaseRequestState() override {
    const size_t released = GroupbyAggregator::releaseRequestState() +
        static_cast<size_t>(decodedSum_ != nullptr) +
...
struct GroupbyCollectSetAggregator : GroupbyAggregator {
  ...
  void addGroupbyRequest(
    ...
    VELOX_CHECK(!maskIndex.has_value(), "collect_set does not support masks");
    auto& request = requests.emplace_back();
    outputIdx_ = requests.size() - 1;
    request.values = tbl.column(inputIndex);
...
struct GroupbyCollectListAggregator : GroupbyAggregator {
  ...
  void addGroupbyRequest(
    ...
    VELOX_CHECK(!maskIndex.has_value(), "collect_list does not support masks");
    auto& request = requests.emplace_back();
    outputIdx_ = requests.size() - 1;
    request.values = tbl.column(inputIndex);
    if (exec::isRawInput(step)) {
  ...
  size_t releaseRequestState() override {
    const size_t released = GroupbyAggregator::releaseRequestState() +
        (nonNullMergeInput_ == nullptr ? 0 : 1);
    nonNullMergeInput_.reset();
    return released;
  }
...
void CudfGroupby::computeFinalGroupbyDynamically(CudfVectorPtr input) {
...
void CudfGroupby::computeFinalGroupbyStreaming(CudfVectorPtr tbl) {
  VELOX_CHECK(tbl->stream().get() == stateStream_.get());
  const auto representedRows = static_cast<uint64_t>(tbl->size());
  auto preparedInput = prepareAggregationInput(
      tbl->getTableView(),
      static_cast<cudf::size_type>(tbl->size()),
  ...
  for (auto& aggregator : aggregators_) {
    aggregator->addGroupbyRequest(
        permutedInputView, regularRequests, stateStream_, get_temp_mr());
  }
...
CudfGroupby::FinalAggregationRun CudfGroupby::mergeFinalAggregationRuns(
  ...
  VELOX_CHECK(left.data->stream().get() == stateStream_.get());
  VELOX_CHECK(right.data->stream().get() == stateStream_.get());
...
void CudfGroupby::doAddInput(RowVectorPtr input) {
  ...
  if (streamingGroupbyEnabled_) {
    computeFinalGroupbyDynamically(std::move(cudfInput));
    return;
  }
