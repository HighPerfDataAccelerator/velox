...
cudf::table_view makeGeneralPartitionKeys(
...
std::unique_ptr<cudf::column> computeGeneralRowNumbers(
...
bool CudfTopNRowNumber::useBoundedTop1(
...
CudfTopNRowNumber::CudfTopNRowNumber(
    ...
    : CudfOperatorBase(
      ...
      boundedTop1_(useBoundedTop1(node)),
  ...
  if (!boundedTop1_) {
    generalPartitionKeys_.reserve(partitionKeys_.size());
    for (size_t i = 0; i < partitionKeys_.size(); ++i) {
      generalPartitionKeys_.push_back(static_cast<cudf::size_type>(i));
    }
    generalCudaEvent_ = std::make_unique<CudaEvent>(cudaEventDisableTiming);
  }
}
...
void CudfTopNRowNumber::doAddInput(RowVectorPtr input) {
  if (!boundedTop1_) {
    addInputGeneral(std::move(input));
    return;
  }
  VELOX_CHECK_NULL(
      pendingInput_,
...
CudfVectorPtr CudfTopNRowNumber::reduceBatchToLocalCandidates(
  ...
  auto keyTable = inputView.select(allKeyIndices_);
  auto indices = cudf::stable_sorted_order(
      keyTable, columnOrders_, nullOrders_, stream, mr);
  ...
  auto localCandidatesTable = cudf::gather(
      inputView,
      filteredIndicesTable->view().column(0),
      cudf::out_of_bounds_policy::DONT_CHECK,
      cudf::negative_index_policy::NOT_ALLOWED,
      stream,
      mr);
  const auto size = localCandidatesTable->num_rows();
...
void CudfTopNRowNumber::addInputGeneral(RowVectorPtr input) {
  if (limit_ == 0 || input->size() == 0) {
    return;
  }
  auto cudfInput = std::dynamic_pointer_cast<CudfVector>(input);
  VELOX_CHECK_NOT_NULL(cudfInput, "CudfTopNRowNumber expects CudfVector");
  auto localCandidates = reduceBatchToLocalCandidates(
      cudfInput, cudfInput->stream(), get_output_mr());
  if (generalCandidates_ == nullptr) {
    generalCandidates_ = std::move(localCandidates);
    return;
  }
  auto mergeStream = cudfGlobalStreamPool().get_stream();
  generalCandidates_ = mergeAndPruneCandidates(
      generalCandidates_, localCandidates, mergeStream, get_output_mr());
}

void CudfTopNRowNumber::noMoreInputGeneral() {
...
RowVectorPtr CudfTopNRowNumber::getOutputGeneral() {
  ...
  if (!generateRowNumber_) {
    return std::move(generalCandidates_);
  }
