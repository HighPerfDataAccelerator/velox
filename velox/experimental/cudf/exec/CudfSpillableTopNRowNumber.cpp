...
void synchronizeTopNPhase(
  ...
  if (error != cudaSuccess) {
    VELOX_FAIL(
        "CudfSpillableTopNRowNumber node={} inputBatches={} phase={} CUDA stream "
...
bool CudfSpillableTopNRowNumber::shouldReplace(
...
CudfSpillableTopNRowNumber::CudfSpillableTopNRowNumber(
    int32_t operatorId,
  ...
  // Keep the community operator as the public execution identity.  The
  // runtime marker makes the automatically selected bounded-memory backend
  // observable without exposing a second user-facing operator.
  addRuntimeStat("topNSpillableBackend", RuntimeCounter(1));
  VELOX_CHECK_EQ(limit_, 1, "CudfSpillableTopNRowNumber only supports limit=1");
  VELOX_CHECK_GT(
      candidateRunBytes_,
      0,
      "CudfSpillableTopNRowNumber candidate run bytes must be greater than zero");
  VELOX_CHECK_GE(
      hostPartitionCount_,
      2,
      "CudfSpillableTopNRowNumber host partition count must be at least two");
  VELOX_CHECK_GT(
      finalizeInputBytes_,
      0,
      "CudfSpillableTopNRowNumber finalize input bytes must be greater than zero");
  VELOX_CHECK_GT(
      hostResidentBytesLimit_,
      0,
      "CudfSpillableTopNRowNumber host resident byte limit must be greater than zero");
  VELOX_CHECK_GT(
      outputChunkBytes_,
      0,
      "CudfSpillableTopNRowNumber output chunk bytes must be greater than zero");
  VELOX_CHECK_GT(
      maxOutputRows_,
      0,
      "CudfSpillableTopNRowNumber max output rows must be greater than zero");
  VELOX_CHECK_EQ(
      node->rankFunction(),
      core::TopNRowNumberNode::RankFunction::kRowNumber,
      "CudfSpillableTopNRowNumber only supports row_number");
  LOG(INFO) << "CudfSpillableTopNRowNumber node=" << diagnosticNodeId_
...
exec::BlockingReason CudfSpillableTopNRowNumber::isBlocked(
...
void CudfSpillableTopNRowNumber::doAddInput(RowVectorPtr input) {
  VELOX_CHECK_NULL(
...
void CudfSpillableTopNRowNumber::processPendingInput(RowVectorPtr input) {
  ...
  if (partialOutput_ && !generateRowNumber_ && diagnosticInputRows_ == 0 &&
    ...
    logDeviceMemorySnapshot(fmt::format(
        "operator=CudfSpillableTopNRowNumber node={} state=partialSample.begin "
    ...
    logDeviceMemorySnapshot(fmt::format(
        "operator=CudfSpillableTopNRowNumber node={} state=partialSample.reduced "
    ...
    if (static_cast<uint64_t>(sampleOutputRows) * 100 >=
      ...
      logDeviceMemorySnapshot(fmt::format(
          "operator=CudfSpillableTopNRowNumber node={} state=partialSample.released "
      ...
      LOG(WARNING) << "CudfSpillableTopNRowNumber node=" << diagnosticNodeId_
...
void CudfSpillableTopNRowNumber::doNoMoreInput() {
  ...
  if (!candidates_ && passthroughOutputs_.empty()) {
    finished_ = !partitionedRowNumberMode_;
  }
}

RowVectorPtr CudfSpillableTopNRowNumber::doGetOutput() {
  if (pendingInput_ != nullptr) {
    VELOX_CHECK(
        inputWorkspaceAdmission_.has_value() ||
            (partialOutput_ && abandonedPartial_),
  ...
  auto result = computeLimitOneRowNumber(input->view(), stream, mr);
  finished_ = true;
  return result;
}

std::unique_ptr<cudf::table> CudfSpillableTopNRowNumber::reduceToCandidates(
    cudf::table_view input,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr) {
  auto reduced = computeLimitOneRowNumber(input, stream, mr);
...
std::unique_ptr<cudf::table> CudfSpillableTopNRowNumber::reduceOwnedCandidates(
    std::unique_ptr<cudf::table> input,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr) {
  VELOX_CHECK_NOT_NULL(input);
  if (!topNUniquePartitionFastPathEnabled() || partitionKeys_.empty()) {
...
void CudfSpillableTopNRowNumber::externalizeSequentialUniqueCandidates(
...
void CudfSpillableTopNRowNumber::maybeSwitchSequentialDenseInputMode(
    rmm::cuda_stream_view stream) {
...
void CudfSpillableTopNRowNumber::switchSequentialDenseInputMode(
    rmm::cuda_stream_view /*stream*/) {
...
void CudfSpillableTopNRowNumber::spillSequentialDensePrefixDeviceState(
    rmm::cuda_stream_view stream,
    bool flushAll) {
...
void CudfSpillableTopNRowNumber::externalizeRowNumberCandidates(
    std::unique_ptr<cudf::table> candidates,
    rmm::cuda_stream_view stream) {
  VELOX_CHECK_NOT_NULL(candidates);
  if (candidates->num_rows() == 0) {
...
void CudfSpillableTopNRowNumber::storeHostPartitionChunk(
...
void CudfSpillableTopNRowNumber::retainDevicePartitionChunk(
...
bool CudfSpillableTopNRowNumber::canReclaim() const {
  return partitionedDeviceBytes_ > 0;
}

bool CudfSpillableTopNRowNumber::reclaimableBytes(
    uint64_t& reclaimableBytes) const {
  reclaimableBytes = partitionedDeviceBytes_;
  return true;
}

void CudfSpillableTopNRowNumber::reclaim(
  ...
  LOG(WARNING) << "CudfSpillableTopNRowNumber task=" << taskId()
               << " node=" << planNodeId()
...
void CudfSpillableTopNRowNumber::handoffDevicePartitionChunk(
    DevicePackedChunk& chunk,
...
void CudfSpillableTopNRowNumber::spillDevicePartition(
    size_t bucket,
    rmm::cuda_stream_view stream) {
  spillDevicePartitions({bucket}, stream);
}

void CudfSpillableTopNRowNumber::spillDevicePartitions(
...
void CudfSpillableTopNRowNumber::evictLargestDevicePartitions(
    rmm::cuda_stream_view stream) {
  if (deviceResidentBytesLimit_ == 0 ||
...
uint64_t CudfSpillableTopNRowNumber::partitionBytes(size_t bucket) const {
  VELOX_CHECK_LT(bucket, partitionedRowNumberHost_.size());
...
CudfSpillableTopNRowNumber::prepareHostChunkForBulkRestore(HostPackedChunk chunk) {
  if (chunk.data) {
...
void CudfSpillableTopNRowNumber::materializeDiskChunkInto(
    HostPackedChunk chunk,
    uint8_t* destination) {
...
CudfPackedHostRestoreChunk CudfSpillableTopNRowNumber::materializeHostChunk(
    HostPackedChunk chunk) {
  if (chunk.data) {
...
CudfSpillableTopNRowNumber::restoreHostChunk(
    HostPackedChunk chunk,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr,
...
void CudfSpillableTopNRowNumber::repartitionOversizedHostBucket(
    size_t bucket,
...
uint64_t CudfSpillableTopNRowNumber::storeRepartitionedChildrenWave(
    std::unique_ptr<cudf::table> partitioned,
    std::vector<cudf::size_type> offsets,
    size_t childStart,
    rmm::cuda_stream_view stream,
...
CudfSpillableTopNRowNumber::PartitionedOutputResult
CudfSpillableTopNRowNumber::computeNextPartitionedRowNumberOutput() {
...
uint64_t CudfSpillableTopNRowNumber::estimateFinalizeWorkspaceBytes(
...
void CudfSpillableTopNRowNumber::stagePartitionedOutput(
...
CudfVectorPtr CudfSpillableTopNRowNumber::takeNextPartitionedOutputBatch() {
...
void CudfSpillableTopNRowNumber::reclaimCompletedDeviceOutputs() {
...
void CudfSpillableTopNRowNumber::reclaimCompletedFinalizeOwnerships() {
...
void CudfSpillableTopNRowNumber::clearPartitionedRowNumberState() {
...
void CudfSpillableTopNRowNumber::ensureSpillDirectory() {
  if (!spillDirectory_.empty()) {
    return;
  }
  ...
  VELOX_CHECK(
      !taskSpillRoot.empty(),
      "CudfSpillableTopNRowNumber requires an explicit Task spill directory");
...
void CudfSpillableTopNRowNumber::cleanupSpillFiles() {
  if (spillDirectory_.empty()) {
    return;
  }
  std::error_code error;
  std::filesystem::remove_all(spillDirectory_, error);
  if (error) {
    LOG(ERROR) << "Failed to remove CudfSpillableTopNRowNumber spill directory '"
...
void CudfSpillableTopNRowNumber::doClose() {
...
CudfVectorPtr CudfSpillableTopNRowNumber::computeLimitOneRowNumber(
    cudf::table_view input,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr) {
...
CudfSpillableTopNRowNumber::computeGroupedLexicographicTop(
    cudf::table_view input,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr,
    bool preserveTies) {
