...
TEST_F(CudfTopNRowNumberTest, rowNumberHostStateDoesNotUseDisk) {
...
TEST_F(CudfTopNRowNumberTest, earlyCloseCleansHostState) {
  const auto spillRoot = TempDirectoryPath::create();
  const auto preexistingTopLevelSpills = findTopLevelProcessSpillDirectories();
  auto cursor = makeSpillingCursor(spillRoot->getPath());
  ASSERT_TRUE(cursor->moveNext());
...
TEST_F(CudfTopNRowNumberTest, groupedDescendingNullsLastAcrossBatches) {
  ...
  auto first = makeRowVector(
      ...
      {makeNullableFlatVector<std::string>(
       ...
       makeNullableFlatVector<int64_t>({1, 7, 5, std::nullopt, 2, std::nullopt}),
  ...
  auto second = makeRowVector(
      ...
      {makeNullableFlatVector<std::string>(
       ...
       makeNullableFlatVector<int64_t>({9, std::nullopt, 3, 4, std::nullopt, 8}),
       makeFlatVector<std::string>(
...
TEST_F(CudfTopNRowNumberTest, multiKeyArgMaxIsStableAcrossBatches) {
  auto first = makeRowVector(
      {"p", "s1", "s2", "v"},
  ...
  auto second = makeRowVector(
      ...
      {makeFlatVector<std::string>({"a", "a", "b", "c", "c"}),
       makeNullableFlatVector<int64_t>(
           {5, 4, 7, std::nullopt, std::nullopt}),
       makeNullableFlatVector<int64_t>({3, 9, 4, 5, 7}),
       makeFlatVector<std::string>(
           {"a-later-batch", "a4", "b7", "c5", "c7"})});
  ...
  auto plan =
      PlanBuilder()
          ...
          .topNRowNumber(
              {"p"},
              {"s1 DESC NULLS LAST", "s2 DESC NULLS LAST"},
              1,
              true)
          .planNode();
  const auto spillRoot = TempDirectoryPath::create();
...
TEST_F(CudfTopNRowNumberTest, uniquePartitionFastPathIsExactAndFallsBack) {
  ...
  EXPECT_GT(
      topNStats.customStats.at("topNUniquePartitionFallbackRows").sum, 0);
  ...
  if (sequential != nullptr && std::string_view(sequential) == "1") {
    ...
    if (sparseEnabled) {
      EXPECT_EQ(
          topNStats.customStats.at("topNSequentialDuplicateKeyRows").sum,
          3);
      EXPECT_EQ(
          topNStats.customStats.at("topNSequentialSparseCandidateRows").sum,
          6);
      EXPECT_EQ(
          topNStats.customStats.at("topNSequentialSingletonOutputRows").sum,
          1);
      EXPECT_EQ(
          topNStats.customStats.count("topNSequentialUniqueFallbackRows"),
          0);
    } else {
      EXPECT_GT(
          topNStats.customStats.at("topNSequentialUniqueFallbackRows").sum,
          0);
    }
  }
  EXPECT_TRUE(findSpillDirectories(spillRoot->getPath()).empty());
...
TEST_F(
  ...
  auto expected = makeRowVector(
      ...
      {makeFlatVector<int64_t>(
           kKeys, [](vector_size_t row) { return row; }),
       makeFlatVector<int64_t>(
           kKeys,
           [](vector_size_t row) { return kRows - kKeys + row; }),
       makeFlatVector<std::string>(
           kKeys,
           [](vector_size_t row) {
             const auto sourceRow = kRows - kKeys + row;
             return std::string(
                 256, static_cast<char>('a' + sourceRow % 26));
           }),
       makeFlatVector<int64_t>(
           kKeys, [](vector_size_t) { return 1; })});
  assertEqualResults({expected}, actual);
  const auto planStats = exec::toPlanStats(cursor->task()->taskStats());
  const auto& topNStats = planStats.at(plan->id());
  EXPECT_EQ(
      topNStats.customStats
          .at("topNSequentialProvenDuplicateCandidateRows")
          .sum,
      kRows);
  EXPECT_EQ(
  ...
  EXPECT_EQ(
      topNStats.customStats
          .at("topNSequentialSparseDenseFallbackCandidateRows")
          .sum,
      kRows);
  EXPECT_EQ(
      topNStats.customStats.at("topNSequentialUniqueFallbackRows").sum,
      kRows);
  EXPECT_GT(
      topNStats.customStats.at("topNSequentialDenseRawRewriteBytes").sum,
      0);
  EXPECT_GT(
      topNStats.customStats.at("topNSequentialDenseRawRewriteChunks").sum,
      0);
...
TEST_F(CudfTopNRowNumberTest, uniquePartitionFastPathSkipsReduction) {
  ScopedEnvVar uniqueFastPath(
      "GLUTEN_CUDF_TOPN_UNIQUE_PARTITION_FAST_PATH", "1");
  auto data = makeRowVector(
      {"p", "s", "v"},
  ...
  if (sequential != nullptr && std::string_view(sequential) == "1") {
    EXPECT_GT(
        ...
        0);
    EXPECT_GT(
        topNStats.customStats.at("topNSequentialUniqueOutputRows").sum,
        0);
    EXPECT_EQ(
        topNStats.customStats.count("topNSequentialUniqueFallbackRows"),
        0);
    ...
    EXPECT_GT(
        topNStats.customStats.at("topNUniquePartitionFastPathRows").sum,
        0);
    EXPECT_EQ(
...
TEST_F(CudfTopNRowNumberTest, DISABLED_widePayloadLateGatherBenchmark) {
  ...
  const bool sparseDuplicates = sparseValue != nullptr &&
      std::string_view(sparseValue) == "1";
  ...
  auto data = makeRowVector(
      ...
      {makeFlatVector<int64_t>(
           ...
           [sparseDuplicates](auto row) {
             return sparseDuplicates && row % kDuplicateEvery ==
                     kDuplicateEvery - 1
                 ? row - 1
                 : row;
           }),
       ...
       makeFlatVector<std::string>(
           kRows, [&](auto row) { return payload + std::to_string(row % 79); })});
  ...
  params.queryConfigs = {
      ...
      {cudf_velox::CudfConfig::kCudfTopNRowNumberOutputChunkBytes,
       benchmarkOutputBytes == nullptr ? "33554432"
  ...
  const auto expectedRows = sparseDuplicates
      ? kRows - kRows / kDuplicateEvery
      : kRows;
  EXPECT_EQ(outputRows, expectedRows);
  const auto taskStats = cursor->task()->taskStats();
  EXPECT_TRUE(usedCudfTopNRowNumber(taskStats));
  for (const auto& pipelineStats : taskStats.pipelineStats) {
    for (const auto& operatorStats : pipelineStats.operatorStats) {
      if (operatorStats.operatorType == "CudfTopNRowNumber") {
        ...
        std::cout << "wide_payload_topn sparse_duplicates="
                  << sparseDuplicates << " add_input_wall_ms="
                  ...
                  << runtimeSum("topNPageableDirectRestoreBytes") /
                      (1024.0 * 1024.0)
                  << " host_stage_ms="
                  << runtimeSum("topNPinnedBounceHostStageNanos") /
                      1'000'000.0
                  << " copy_sync_ms="
                  ...
                  << " unique_probe_ms="
                  << runtimeSum("topNUniquePartitionProbeNanos") /
                      1'000'000.0
                  << " unique_fast_rows="
                  ...
                  << " sequential_probe_ms="
                  << runtimeSum("topNSequentialUniqueProbeNanos") /
                      1'000'000.0
                  << " sequential_key_mib="
                  << runtimeSum("topNSequentialKeyBytes") /
                      (1024.0 * 1024.0)
                  ...
                  << runtimeSum("topNSequentialSparseHostStageNanos") /
                      1'000'000.0
                  << " sequential_sparse_copy_sync_ms="
                  << runtimeSum("topNSequentialSparseCopySyncNanos") /
                      1'000'000.0
                  << " sequential_sparse_split_ms="
                  << runtimeSum("topNSequentialSparseSplitNanos") /
                      1'000'000.0
                  << " sequential_direct_chunk_mib="
                  << runtimeSum("topNSequentialDirectChunkBytes") /
                      (1024.0 * 1024.0)
                  << " sequential_direct_chunk_batches="
                  << runtimeSum("topNSequentialDirectChunkBatches")
                  << " spill_input_mib="
                  << runtimeSum("topNSpillInputBytes") /
                      (1024.0 * 1024.0)
                  << " spill_stored_mib="
                  << runtimeSum("topNSpillStoredBytes") /
                      (1024.0 * 1024.0)
                  << " spill_compression_ms="
                  << runtimeSum("topNSpillCompressionNanos") /
                      1'000'000.0
                  << " spill_compressed_chunks="
                  << runtimeSum("topNSpillCompressedChunks")
                  << " spill_raw_chunks="
                  << runtimeSum("topNSpillRawChunks")
                  << std::endl;
      }
    }
  }
...
TEST_F(CudfTopNRowNumberTest, abandonsLowReductionPartial) {
  ...
  auto partial =
      core::TopNRowNumberNode::Builder(*basePartial)
          .id("partial")
          .partialOutput(true)
          .build();
  auto final =
  ...
  auto expected = makeRowVector(
      ...
      {makeFlatVector<int64_t>(
       ...
       makeFlatVector<int64_t>(
           2 * kRows, [](vector_size_t row) { return row; }),
       makeFlatVector<int64_t>(
           2 * kRows, [](vector_size_t) { return 1; })});
  assertEqualResults({expected}, actual);
  ...
  ASSERT_EQ(partialStats.customStats.at("partialAbandonSampleInputRows").sum, 64);
  ASSERT_EQ(
...
TEST_F(
  ...
  auto first = makeRowVector(
      ...
      {makeFlatVector<int64_t>(
       ...
       makeFlatVector<std::string>(
           kRowsPerBatch,
           [](vector_size_t row) { return "first-" + std::to_string(row); })});
  auto second = makeRowVector(
      {"p", "s", "payload"},
      {makeFlatVector<int64_t>(
           kRowsPerBatch, [](vector_size_t row) { return row; }),
...
TEST_F(
        ...
        {makeFlatVector<int64_t>(
         ...
         makeFlatVector<std::string>(
             ...
             [begin](vector_size_t row) {
               ...
               return std::string(
                          256,
                          static_cast<char>('a' + globalRow % 26)) +
      ...
      {makeFlatVector<int64_t>(
       ...
       makeFlatVector<std::string>(
           ...
           [](vector_size_t row) {
             ...
             return std::string(
                        256,
                        static_cast<char>('a' + sourceRow % 26)) +
  ...
  const auto* sparse =
      std::getenv("GLUTEN_CUDF_TOPN_SEQUENTIAL_SPARSE_DUPLICATES");
  const bool sparseEnabled =
      sequential != nullptr && std::string_view(sequential) == "1" &&
      (sparse == nullptr ||
       (std::string_view(sparse) != "0" &&
        std::string_view(sparse) != "false" &&
        std::string_view(sparse) != "FALSE"));
...
TEST_F(CudfTopNRowNumberTest, largeFinalizeBucketHasBoundedOutputBatches) {
  ...
  params.queryConfigs = {
      ...
      {cudf_velox::CudfConfig::kCudfDeviceResidentCapacityBytes,
       "1048576"}};
  auto cursor = TaskCursor::create(params);
  std::vector<RowVectorPtr> actual;
  while (cursor->moveNext()) {
  ...
  if (sequential != nullptr && std::string_view(sequential) == "1") {
    ...
    EXPECT_GT(
        topNStats.customStats.at("topNFinalizePreflushBytes").sum, 0);
    EXPECT_GT(
        topNStats.customStats.at("topNGlobalAdmissionRejectedBytes").sum,
        0);
  }
}

TEST_F(CudfTopNRowNumberTest, deviceOutputStagingHonorsConfiguredBounds) {
